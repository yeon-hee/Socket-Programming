...
// -----------------------------------------------------------------//
// Title : Construction Proxy Connection #3-2                       //
//////////////////////////////////////////////////////////////////////
...
void *thr_fn1(void *url)
{
	...
	printf("*PID# %d create the *TID# %u.\n", getpid(), (unsigned int)tid);
	sleep(10);
	fprintf(File, "[HIT] %s/%s\n", hashed_d, hashed_f); // fprintf log_hit
	fprintf(File, "[HIT]%s\n", (char *)url);
	printf("*TID# %u is exited.\n", (unsigned int)tid);
	printf("*PID# %d exited the critical zone.\n", getpid());
			...
			while ((len_out = read(client_fd, buf, BUFFSIZE)) > 0) // request 
			{
				...
				if (pDir == NULL) // MISS - no directory 
				{
					...
					if ((strcmp(arr, "html") == 0) || (arr[3] == '/'))
					{
						...
						err = pthread_create(&tid, NULL, thr_fn, (void*)url); // thread cread
						if (err != 0) // error message
						{
							printf("pthread_create() error.\n");
							return 0;
						}
						pthread_join(tid, &tret); // wait
					}
				...
				{ // read directory
					...
					for (pFile = readdir(pDir); pFile; pFile = readdir(pDir))
					{
						if (!strcmp(hashed_f, pFile->d_name)) // if exist file
						{
							...
							confirm = 1; // hit
							printf("*PID# %d is waiting for the semaphore.\n", getpid()); // waiting for semaphore 
							err = pthread_create(&tid, NULL, thr_fn1, (void*)url); // thread creat
							if (err != 0) // error
							{
								printf("pthread_create() error.\n");
								return 0;
